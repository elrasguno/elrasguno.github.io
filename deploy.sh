...
HOME_PATH=$HOME
GHOST_PATH="${PWD:-${HOME_PATH}/.ghost/}"
...
first_deploy() {
	if [ -d "$GHOST_PATH" ]; then
		...
		buster generate --domain="$GHOST_SERVER_URL" --dir="${STATIC_DIR}"

		# Fixing Links
		find ${STATIC_DIR} -name *.html -type f -exec sed -i '''s#http://localhost:2368#'$gh_username'.github.io/'$gh_repo'#g' {} \;
